 ...
 * WordPress edition: search URLs use /recipes/slug/ permalinks
 */
(function () {
  'use strict';
  ...
  function initActiveNavLink() {
    ...
    document.querySelectorAll('.nav-primary a, .nav-mobile a, .main-nav a').forEach(function (link) {
      ...
      if (!href) return;
      if (path === href || (href !== '/' && path.indexOf(href) !== -1)) {
  ...
  /* ----------------------------------------------------------------
     9. Recipe Filter (archive/listing page)
  ---------------------------------------------------------------- */
  ...
  /* ----------------------------------------------------------------
     12. Search Overlay
     WordPress edition: URLs use /recipes/slug/ permalinks
     Falls back to WordPress native search (/?s=query) if no match
  ---------------------------------------------------------------- */
  function initSearch() {
    var toggleBtn = document.getElementById('searchToggle');
    if (!toggleBtn) return;

    // WordPress permalink base for recipes
    var siteBase = (typeof marsrecipesData !== 'undefined' && marsrecipesData.homeUrl)
      ? marsrecipesData.homeUrl.replace(/\/$/, '')
      : '';

    // Recipe list — slugs match WordPress CPT slugs under /recipes/
    var RECIPES = [
      { title: 'Easy Creamy Garlic Chicken',          time: '30 min', cat: 'Chicken', slug: 'easy-creamy-garlic-chicken' },
      { title: 'One-Pan Beef Shawarma Bowl',          time: '35 min', cat: 'Beef',    slug: 'one-pan-beef-shawarma-bowl' },
      { title: 'Spicy Garlic Butter Shrimp',          time: '10 min', cat: 'Seafood', slug: 'spicy-garlic-butter-shrimp' },
      { title: 'Crispy Honey Garlic Salmon',          time: '25 min', cat: 'Seafood', slug: 'crispy-honey-garlic-salmon' },
      { title: 'One-Pan Honey Butter Chicken',        time: '30 min', cat: 'Chicken', slug: 'one-pan-honey-butter-chicken' },
      { title: 'Garlic Butter Steak Bites',           time: '15 min', cat: 'Beef',    slug: 'garlic-butter-steak-bites' },
      { title: 'Creamy Tuscan Shrimp',                time: '20 min', cat: 'Seafood', slug: 'creamy-tuscan-shrimp' },
      { title: 'Lemon Herb Sheet Pan Chicken',        time: '45 min', cat: 'Chicken', slug: 'lemon-herb-sheet-pan-chicken' },
      { title: 'Crispy Baked Chicken Wings',          time: '50 min', cat: 'Chicken', slug: 'crispy-baked-chicken-wings' },
      { title: 'Ground Beef Kofta with Garlic Sauce', time: '30 min', cat: 'Beef',    slug: 'ground-beef-kofta-garlic-sauce' },
      { title: 'Coconut Chicken Curry',               time: '35 min', cat: 'Chicken', slug: 'coconut-chicken-curry' },
      { title: 'Creamy Sun-Dried Tomato Pasta',       time: '25 min', cat: 'Pasta',   slug: 'creamy-sun-dried-tomato-pasta' },
      { title: 'Beef & Broccoli Stir Fry',            time: '20 min', cat: 'Beef',    slug: 'beef-broccoli-stir-fry' },
      { title: 'Smoky Paprika Baked Salmon',          time: '25 min', cat: 'Seafood', slug: 'smoky-paprika-baked-salmon' },
      { title: 'Easy Chicken Tikka Masala',           time: '40 min', cat: 'Chicken', slug: 'easy-chicken-tikka-masala' }
    ];
    ...
    function renderResults(raw) {
      ...
      if (!list.length) {
        // Fall back to WordPress search
        results.innerHTML =
          '<p class="search-overlay__empty">No recipes found for "<strong>' + escHtml(q) + '</strong>". ' +
          '<a href="' + siteBase + '/?s=' + encodeURIComponent(q) + '" style="color:inherit;text-decoration:underline">Search the site →</a></p>';
        return;
      }

      results.innerHTML = list.map(function (r, i) {
        return '<a class="search-result-item" href="' + recipeUrl(r.slug) + '" role="option" tabindex="-1" data-idx="' + i + '">' +
